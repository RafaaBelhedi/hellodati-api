...
class ExtraPostsTranslate extends Model
{
    public function post()
    {
        return $this->belongsTo('App\ExtraPost')->withDefault();
    }
    ...
    public function setSelf($request){
        if(array_key_exists ("post_id",$request)){
            ...
            try
            {
                $post = ExtraPost::findOrFail(intval($request["post_id"]));
                $post_id=$post->id;
            }
    ...
    public static function getEditableColumns(){
        ...
        $table_info_columns  = (array)DB::select( DB::raw('SHOW FULL COLUMNS FROM '.app(ExtraPostsTranslate::class)->getTable().''));
    ...
    public static function Finder($request_data){
        $postTranslate=new ExtraPostsTranslate();
